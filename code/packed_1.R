...
BiocManager::install(c("SingleCellExperiment", "SummarizedExperiment"), ask = FALSE)

library(SummarizedExperiment)
library(SingleCellExperiment)

# Fixed reorder_matrix_real
reorder_matrix_real <- function(real_sce){
  counts_matrix <- assay(real_sce, "counts")
  counts_df_real <- data.frame(Gene = rownames(counts_matrix), counts_matrix)
  colnames(counts_df_real) <- sub("^X", "", colnames(counts_df_real))
...
# Batch process all RDS files and apply your functions
process_all_sce <- function(folder_path){
  # List all .rds files
  rds_files <- list.files(path = folder_path, pattern = "\\.rds$", full.names = TRUE)
  
  processed_list <- lapply(rds_files, function(file_path){
    # 1. Read RDS
    sce_obj <- readRDS(file_path)
    
    # 2. Reorder matrix
    counts_df <- reorder_matrix_real(sce_obj)
    
    # 3. Create new SCE
    sce_new <- create_sce(counts_df)
    
    # 4. Compute logcounts
    sce_new <- compute_logcounts(sce_new)
    
    # 5. Optional: run PCA
    sce_new <- computePCA(sce_new)
    
    return(sce_new)
  })
  
  # Name the list by file names
  names(processed_list) <- sub("\\.rds$", "", basename(rds_files))
  return(processed_list)
}

# Example usage
folder_path <- "/Users/oukanyou/Desktop/SpatialSimBench/data"
processed_sce <- process_all_sce(folder_path)

# Check results
names(processed_sce)      # Names of each SCE object
processed_sce[[1]]        # Inspect first SCE
assay(processed_sce[[1]], "counts")   # Counts matrix
assay(processed_sce[[1]], "logcounts") # Logcounts
