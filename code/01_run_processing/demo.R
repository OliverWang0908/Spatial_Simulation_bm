...
file_path = "/Users/oukanyou/Desktop/SpatialSimBench/data/BREAST.rds"
sce_obj <- readRDS(file_path)
sce_obj <- compute_logcounts(sce_obj)
sce_obj <- computePCA(sce_obj)

counts_df_real <- reorder_matrix_real(sce_obj)

### so far so good -- Spatial data processed
...
for (file_path in rds_files) {
  ...
  # ------------------------------------------------------------
  rds_obj <- readRDS(file_path)
  sce_obj <- create_sce(rds_obj)
  counts_df_real <- reorder_matrix_real(sce_obj)
